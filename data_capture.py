...
import urllib,datetime,sqlite3 as lite,data_processing#, pandas as pd
...
class Ticker(object):
    ...
    def __init__(self):
        self.symbol = ''
        self.db_path = ''
        self.date,self.time,self.date_time,self.open_,self.high,self.low,self.close,self.volume,self.adj_close = ([] for _ in range(9))
...
#        else:
            if source == 'yahoo':
                for bar in xrange(0,len(csv)-1):
                    ...
                    ds = ds.rstrip().split('-')
                    open_,high,low,close,volume,adj_close = [x.replace('-', '0') for x in [open_,high,low,close,volume,adj_close]]
